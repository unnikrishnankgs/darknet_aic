...
{
    char labelpath[4096];
    find_replace(path, IMAGES_FOLDER, LABELS_FOLDER, labelpath);
    find_replace(labelpath, "JPEGImages", LABELS_FOLDER, labelpath);
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    find_replace(labelpath, "raw", LABELS_FOLDER, labelpath);
    find_replace(labelpath, ".jpg", ".txt", labelpath);
    find_replace(labelpath, ".jpeg", ".txt", labelpath);
    find_replace(labelpath, ".png", ".txt", labelpath);
    find_replace(labelpath, ".JPG", ".txt", labelpath);
...
matrix load_regression_labels_paths(char **paths, int n)
{
    ...
    for(i = 0; i < n; ++i){
        char labelpath[4096];
        find_replace(paths[i], IMAGES_FOLDER, "targets", labelpath);
...
matrix load_tags_paths(char **paths, int n, int k)
{
    ...
    for(i = 0; i < n; ++i){
        char label[4096];
        find_replace(paths[i], "imgs", LABELS_FOLDER, label);
        find_replace(label, "_iconl.jpeg", ".txt", label);
        FILE *file = fopen(label, "r");
        if(!file){
            find_replace(label, LABELS_FOLDER, "labels2", label);
            file = fopen(label, "r");
            if(!file) continue;
        }
        ++count;
...
{
    char labelpath[4096];
    find_replace(path, IMAGES_FOLDER, "mask", labelpath);
...
data load_data_compare(int n, char **paths, int m, int classes, int w, int h)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        char imlabel1[4096];
        char imlabel2[4096];
        find_replace(paths[i*2],   "imgs", LABELS_FOLDER, imlabel1);
        ...
        find_replace(paths[i*2+1], "imgs", LABELS_FOLDER, imlabel2);
